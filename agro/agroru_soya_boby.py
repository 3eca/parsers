...
#
# def write_csv(data):
#     name_file = 'agroru_soya_boby.csv'
...
def write_in_sql(data):
    ...
    write_sql = '''INSERT INTO SoyaBoby (Ad, Price, Region, Date) VALUES ('%s', '%s', '%s', '%s', '%s')''' % \
...
def parser_html(html):
    ...
    for ad in ads:
        ...
        if price != '':
            ...
            lower = message.lower()
            find_tovar = re.compile('(соевые бобы)|(соя бобы)|(бобы соя)')
...
def soya_sell():
    url = 'https://agroru.com/doska/soya/ct-0-p1.htm'
    base_part_url = 'https://agroru.com/doska/soya/ct-0-p'
    end_part_url = '.htm'
...
def soya_buy():
    url = 'https://agroru.com/spros/soya/ct-0-p1.htm'
    base_part_url = 'https://agroru.com/spros/soya/ct-0-p'
    end_part_url = '.htm'
    total_pages_2 = get_total_pages(get_html(url, headers))
    for i in range(1, total_pages_2):
        url_generation = base_part_url + str(i) + end_part_url
        html = get_html(url_generation, headers)
        get_urls_ads_sell(html)

    count = 0
    ads_urls_set_sell = set(ads_urls_list_sell)
    for urls in ads_urls_set_sell:
        count += 1
        parser_html(get_html(urls, headers))
        # print(urls)
        # print('Parsing url sell %d%%' % (count / len(ads_urls_set_sell) * 100))
