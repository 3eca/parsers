...
# def write_csv(data):
#     name_file = 'rynok_apk_soya_boby.csv'
...
def write_in_sql(data):
    ...
    write_sql = '''INSERT INTO SoyaBoby (Ad, Price, Region, Date) VALUES ('%s', '%s', '%s', '%s')''' % \
...
def parser_html(html):
    ...
    for ad in ads:
        ...
        if price != '':
            ...
            find_product = re.search('(соевые бобы)|(соя бобы)|(бобы соя)', message)
...
def buy():
    url = 'https://rynok-apk.ru/billboard/buy-bobovye/soya/'
...
def sell():
    url = 'https://rynok-apk.ru/billboard/bobovye/soya/'
